...
This repo is a **marketplace with one plugin, `inbound-ads` (11 skills)**, covering Inbound CPH's full Google Ads lifecycle. Its skills group into three jobs:
...
- **Optimize a LIVE account** — `inb-ads-search-term-analyse` (merged from the former soegeterm-analyse + search-term), `inb-ads-rsa-hygiene`, `inb-ads-quality-score`, `inb-ads-display-placement-audit` (RSA asset-hygiene, search-terms, Quality Score deep-dive, GDN placement junk-audit).
...
External write means: anything that mutates a file in Drive, sends an email, posts to Slack, modifies a Sheet/Doc, or calls a third-party API with side effects — **including any write to a Google Ads account**. **Direct Google Ads writes are allowed only through the `ads-writer` agent, and only per-action HITL-confirmed** (direction set 2026-06-19) — no skill calls a Google Ads write tool itself. **`inb-ads-campaign-build` now creates campaigns directly by default** (2026-07-01): it routes campaign creation through `ads-writer`, HITL-gated per action, started **paused** (safe, recommended) or **active** per the user's choice. Its 10-tab Excel review workbook is now **opt-in** — used only when the client must approve the setup first, after which a human imports the confirmed workbook into Google Ads Editor by hand (or Vej A is run on the approved setup to create it directly). `inb-ads-quality-score` routes its confirmed bid/keyword fixes through `ads-writer` the same per-action HITL way (its landing-page and creative fixes are recommend-only, not account writes). **Budget writes stay gated behind the write-guardrail hook + `INBOUND_ADS_BUDGET_GUARDRAIL`, requiring explicit per-action confirmation.** `inb-ads-display-placement-audit` also routes its confirmed negative placements through `ads-writer`.
...
Pure transforms that never touch a specific client's account or context may skip this — but if a skill takes a client name as input, it preloads.
...
| `inb-ads-quality-score` | Actionable Quality Score deep-dive: QS at keyword grain, worst-keyword clustering, each weak component mapped to its lever; confirmed bid/keyword fixes via `ads-writer`, LP/creative recommend-only | Gated — direct Google Ads write via `ads-writer`, per-action HITL (LP/creative fixes are recommend-only) |
...
All skills now live in one plugin (`inbound-ads`), so `${CLAUDE_PLUGIN_ROOT}` resolves to a directory they all share — no cross-plugin file-reference limitation applies. `inb-ads-campaign-build` reuses `inb-ads-rsa-copy`'s RSA engine directly. The `inb-ads-rsa-hygiene` ↔ `inb-ads-rsa-copy` gap-brief loop is manual paste (no code coupling) between two sibling skills, so it closes the build→operate→iterate loop with a single plugin installed.
...
- **`ads-analyst`** — read-only account analyst. Every diagnostic skill (`inb-ads-account-audit`, `inb-ads-search-term-analyse`, `inb-ads-rsa-hygiene`, `inb-ads-onboarding-analysis`, `inb-ads-quality-score`) dispatches account reading to it. Recommend-only; proposes changes for `ads-writer` to apply, never writes itself.
