...
- **If no report folder is noted yet (find, then let the main skill propose persisting it):** list the client folder's subfolders and pick the report folder by name:
  ```
  parentId = '<Drive-folder id>' and mimeType = 'application/vnd.google-apps.folder' and (title contains 'sentation' or title contains 'statusm' or title contains 'statusrapport')
  ```
  (`title contains 'sentation'` catches "Præsentation" + ASCII "Praesentation"; `statusm` catches "statusmøder"/"statusmoeder".) If several match, take the `#1 -` one; **ignore any titled `OLD - ...`** (legacy). Return the folder (id + exact title + path + the newest deck found inside) for the main skill to confirm and write into the AI-Context file (Trin 5/6). Do NOT write anything from inside the subagent.
- **Extract** from the newest deck: what was reported, what was agreed, named levers/next-steps, any closed/opened håndtag. This is the richest "recently done/agreed" source — but a deck is a point-in-time artifact: feed durable conclusions to the diff, not raw metrics.
- Unparseable/locked deck (`read_file_content` fails or returns empty) → return the link + "kunne ikke læses", never fabricate its contents.
