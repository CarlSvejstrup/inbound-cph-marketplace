...
Læs den shippede skills
`${CLAUDE_SKILL_DIR}/../responsive-search-ads/SKILL.md` — det er motoren du orkestrerer;
...
3. Skriv den ad group's `ads.json` og kør `fill-sheet.py` → ét ark per ad group:
   ```bash
   python3 ${CLAUDE_SKILL_DIR}/../responsive-search-ads/fill-sheet.py \
