# pipeline-flow — campaign-build orkestrering

Dette er kontrakten for hvordan `campaign-build` kører de fire faser, og hvordan hver
...
|---|---|---|
| **pipeline-mode** | `campaign-build`-orkestratoren (denne fil) | Forbruger upstream-JSON fra artefakt-mappen, emitterer sit eget JSON til samme mappe, spørger IKKE om intake den allerede har fået, og stopper IKKE for menneske-godkendelse undervejs (gaten ligger samlet til sidst). |
| **solo-mode** | Et selvstændigt skill (`research`/`structuring`/`assets`) trigget direkte af brugeren | Samme kerne-logik, men samler minimal intake selv hvis nødvendigt, og pakker sit output i en `.xlsx`/`.docx`-rapport til mennesket. Defineret i shell-skillet, ikke her. |
...
1. **Reference-stien** — fx `${CLAUDE_SKILL_DIR}/references/04-structuring.md`. `${CLAUDE_SKILL_DIR}`
   er campaign-build-skillets egen mappe; alle kontrakt-filer ligger som `references/<navn>.md` ved
...
review-workbooken; Editor-CSV'er genereres SENERE af `google-ads-general`-konverteren fra den
