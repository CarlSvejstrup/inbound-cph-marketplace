...
Excel, af `editor-csv-export`-skillen i `google-ads-general`. Derfor er workbooken et **tabsfrit superset**:
...
den til Editor-CSV'er af `google-ads-general`-konverteren, og mennesket importerer + enabler først når alle
...
- **Excel-only:** assembleren emitterer INGEN CSV. CSV-genereringen bor i `google-ads-general`-konverteren.
