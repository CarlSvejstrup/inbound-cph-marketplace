...
description: Lav Google Ads Responsive Search Ad-tekster i hoej kvalitet fra en kundes landingsside, med keyword-data fra Google Ads MCP og udvidet intake (USP, tilbud, trust-tal, brand voice). Aflever et Google Ads Editor-klart regneark navngivet efter Inbounds konvention. Brug naar brugeren siger "lav annoncetekster", "RSA til [klient]", "annonce-ark", "responsive search ad", "tekster ud fra landingsside", eller beder om hoejere kvalitet end annoncetekster v1. Svarer paa dansk.
---

# annoncetekster-v2

Lav Google Ads-annoncetekster (Responsive Search Ads) ud fra en kundes landingsside, en udvidet intake og keyword-data fra Google Ads MCP, og aflever dem i et regneark der kan importeres direkte i Google Ads Editor. Hele forloebet og alt output er paa dansk.

**v2 vs v1:** v1 (`annoncetekster`) skriver copy ud fra landingsside + kampagnenavn. v2 tilfoejer fem ekstra intake-felter (USP-hierarki, aktivt tilbud + udloeb, trust-tal, brand voice/banned words, top-keywords fra MCP) og haandhaever copywriting-reglerne fra `references/headline-craft.md` (angle-taxonomi, Sentence case, laengde-variation, 2026 disapproval-policy). Bruges naar copy-kvaliteten skal vaere hoejere end v1's generiske default.
...
**Sprog: alt foregaar paa dansk** — spørgsmål i intake, statusbeskeder, output-tabellen og naeste-skridt. Skift kun til engelsk hvis brugeren skriver til dig paa engelsk eller udtrykkeligt beder om det. Selve annonceteksterne skrives ogsaa paa dansk (se Trin 4).
...
### B. Copy-inputs (v2 — driver headline-kvaliteten)

Disse fem felter er aarsagen til at v2 eksisterer. Spring dem ALDRIG over — de er forskellen mellem generisk copy og copy der konverterer.

10. **USP-hierarki** — bed brugeren rangere klientens top-3 differentiators. Foreslå options ud fra landingsside-scrape (Trin 2) hvis du allerede har scrapet den; ellers be brugeren skrive dem. Hvis kun een USP er klar: spørg om en sekundaer og tertiaer. Begrund: uden et rangeret USP-hierarki defaulter alle 15 headlines til generelle benefits.

11. **Aktivt tilbud + udloebsdato** — options:
    - "Ja, der er et tilbud" → følg op med tilbudstekst + udloebsdato.
    - "Nej, ingen aktiv promo" → skip urgency-angle i Trin 4.
    Begrund: udloebne tilbud i copy = auto-disapproval. Aktiv promo driver urgency-headline + evt. countdown customizer i Editor.

12. **Trust-tal** — specifikke tal vi maa bruge: kunde-antal, aar i branchen, anmeldelses-score, certificeringer, awards. Options foreslået ud fra landingsside hvis muligt ("4.8 stjerner fra 2.300 anmeldelser", "Foretrukket af 50.000+ danskere", "Etableret 1998"). Hvis ingen tal: spørg om de findes paa Trustpilot eller andetsteds — vi maa IKKE finde paa tal.

13. **Brand voice + banned words** — to delspoergsmaal:
    - **Tone:** options som "Formel", "Venlig og direkte", "Teknisk og praecis", "Energisk og inspirerende".
    - **Banned words:** ord vi IKKE maa bruge (fx konkurrent-brands, juridisk problematiske ord, ord klienten har bedt om at undgaa). Default option: "(ingen)".
...
Scrape URL'en med Firecrawl. Udtraek konkret:
- **Produkt/ydelse** — hvad sælger de.
- **USP-kandidater** — hvad gør de anderledes (bruges til intake-spoergsmaal 10).
- **Tone of voice** — formel, venlig, teknisk, energisk (bruges til intake-spoergsmaal 13).
- **CTA'er paa siden** — hvilke handlinger styrer de mod ("Faa et tilbud", "Bestil demo", "Koeb nu").
- **Trust-signaler med tal** — anmeldelses-score + antal, kunde-antal, aar etableret, certificeringer, awards (bruges til intake-spoergsmaal 12).
- **Pris/tilbud** — hvis et aktivt tilbud staar paa siden (bruges til intake-spoergsmaal 11).
- **Brandnavn og logo-tekst.**
- **Sidens sprog** — hvis ikke dansk, matcher copy sidens sprog.

Hvis siden ikke kan hentes: sig det og stop. Vi opfinder ikke claims.

Hvis du allerede har scrapet siden FOER Trin 1's USP/trust/tilbud-spoergsmaal: brug scrapen til at foreslå konkrete options i `AskUserQuestion` i stedet for friform-svar. Det er hele pointen med v2.

## Trin 3 — Laes copywriting-reglerne

**FOER du skriver copy:** laes `${CLAUDE_PLUGIN_ROOT}/skills/annoncetekster-v2/references/headline-craft.md`. Den indeholder:
...
- Laengde-variation-maal (4-5 korte, 6-7 mellem, 3-4 lange).
- Sentence case-reglen (3.7× CPA-forskel — det er ikke smag).
- Keyword-tilstedevaerelse-kravet (top-keyword i ≥3 headlines).
...
- Kvalitets-check-listen du gennemgaar foer arket bygges.

Disse regler er testet paa millioner af annoncer. Foelg dem.

## Trin 4 — Generer copy

Producer **20-25 headline-kandidater**, derefter vaelg de 15 bedste der opfylder angle-fordelingen fra reference-filen. Plus **4 descriptions** og **2 paths**.

Haarde graenser: headlines ≤ 30 tegn, descriptions ≤ 90, paths ≤ 15.

**Regler (uddybet i `references/headline-craft.md`):**
- Kun claims der staar paa landingssiden eller blev bekraeftet i intake (USP-hierarki, trust-tal). Ingen opfundne tal, garantier eller priser.
- **Sentence case overalt** — ikke Title Case.
- **Top-keyword** (fra Google Ads MCP eller manuelt intake) skal staa i **mindst 3 headlines**.
- **Laengde-variation:** bland korte (<20), mellem (20-26) og lange (27-30).
- **Banned words** fra intake: scan copy'en — ingen optraeden.
- **Sproget:** dansk medmindre landingssiden er paa et andet sprog.
- **Laengde-selvtjek:** for hver streng, taeel tegn. Ret over-laengde INDEN du gaar videre. `fill-sheet.py` afviser ogsaa, men det er hurtigere at fange her.

Gennemgaa kvalitets-check-listen fra reference-filen foer du skriver `copy.json`.
...
Hvis scriptet afviser pga. for lange felter: ret copy'en og koer igen. Output er en `.xlsx` med teksterne i raekke 2, live LEN-formler, roede farveregler og auto-tilpassede kolonne-bredder — klar til kunde-review.
...
- Regenerer template: `python3 ${CLAUDE_PLUGIN_ROOT}/skills/annoncetekster-v2/build-template.py` (kun naar kolonnelayout aendrer sig).
...
- Copywriting-reglerne i `references/headline-craft.md` skal re-checkes hvis Google aendrer disapproval-policy eller Ad Strength-vaegtning. Kilder med tal-driven evidens (Optmyzr-studiet) maa ikke vaere over 12 maaneder gamle uden ny verifikation.
