...
**Format = native Google Doc** — the shared source of truth is `../../shared/ai-context-file-contract.md`; read it before building, and follow its section skeleton so a new file looks identical to the existing ones. The file is a **native Google Doc**, NOT a raw `.md` (do NOT upload `disableConversionToGoogleType: true`). This is load-bearing correctness, not cosmetics: `inb-ads-client-brief` maintains the same file in place via `findAndReplaceInDoc`, which only works on a native Doc. A file published as a raw `.md` cannot be updated by the brief skill — that was a real latent bug this format fixes. Title WITHOUT a `.md` suffix: `<Client Name> - <HubSpotID>` (e.g. `Dantaxi - 7438308806`). If `<HubSpotID>` is missing for a market, use the literal `ingen-HubSpot` in the title (e.g. `GSGroup SE - ingen-HubSpot`). Existing raw `.md` outliers (currently only Deloitte, the new pilot) are converted to a Doc at their next update; new and updated files are always native Docs.

**Content is already-cleaned, timeless context.** The source is each client's `## Klientoverblik` in `work/inbound-cph/clients/<slug>.md`, deliberately stripped of point-in-time performance (no ROAS/CPA/CTR/counts/LAST_30_DAYS). Carry it verbatim — never re-add live numbers. Configured settings (tCPA/budget caps), naming conventions, and structural caveats stay. `Klientoverblik` is timeless-only per `../../shared/ai-context-file-contract.md` (the shared rule). See also the vault-side rule `feedback-inbound-klientoverblik-timeless-only` (Carl's OS vault, not this plugin).

Exclude from the published file: YAML frontmatter; any `### Reconcile-flag` block or `<!-- RECONCILE ... -->` comment (internal audit only); embedded changelog entries or a `## Changelog` section (keep only the changelog_file LINK in the ID-block — the changelog stays a separate doc); the **vault note's own** `## Drive files` / `## Drive-filer` mirror list (Carl: omit that local list entirely — the published file's ranked source-doc index is instead built fresh from the step-3 sweep, per the shared contract's Drive-files slot); empty placeholder sections. Include the ID-block with ClickUp folder ID + HubSpot lifecycle stage (omit the stage line only when the folder is untagged and stage is unknown), plus the durable human sections that exist + full Klientoverblik + the Aftaleark digest.
...
2. **Create/reuse the "AI Context" folder — ALWAYS at the client-folder ROOT.** The folder lives at the top level of the client folder, NOT inside Paid Search or any other subfolder. `search_files parentId='<CLIENT_FOLDER_ID>' and mimeType='application/vnd.google-apps.folder'`. Ian already creates these folders himself and drops his own files in them (e.g. DBI's holds his Projektoverblik + Optimeringslog + Demo-brief) — reuse the existing one, our uniquely-named file sits alongside his. Watch for case variants: Ian used "AI context" (lowercase c) on InboundCPH — reuse it rather than spawn a near-duplicate. Only create a new folder if none exists.
3. **Gather the content.** If a canonical vault note (`work/inbound-cph/clients/<slug>.md`) exists, it is the primary source — carry its `## Klientoverblik` verbatim. For a NEW client with no note yet, originate the context from Drive. **Either way, do not read only the Paid Search folder — sweep the whole client folder for relevant material:** other root subfolders (Strategi og projektledelse, Forprojekt og salg, Grafik og design, Marketing automation, Tracking & CRO, SEO, Paid Social) and the client root itself. Look specifically for onboarding/opstart docs, strategy decks, brand/tone-of-voice + persona/audience docs, the Aftaleark & kundebrief, and any **reports** (status reports, deepdives, slutrapporter). **If any report exists, report it back to the user** (name + link) so they know it is there — surface reports even when they live outside Paid Search. Then build the file per `references/publish-contract.md`. Drop YAML frontmatter; convert `[[wikilinks]]` to plain text; skip empty sections; exclude the Reconcile-flag, embedded changelog entries, and the Drive-filer list per the constraints above.
4. **Create the file ONCE** as a **native Google Doc**: `createDocFromMarkdown` (the native-Doc create path — it renders the markdown headings + the `| Felt | Værdi |` table as native Doc formatting, visually matching the existing files and staying editable in place by `inb-ads-client-brief`'s `findAndReplaceInDoc`), `name: "<Client Name> - <HubSpotID>"` (NO `.md` suffix), `parentFolderId: <AI_CONTEXT_FOLDER_ID>`, `markdown` = the full text built to the shared contract's skeleton. Do NOT pass `disableConversionToGoogleType`. Search the folder for that exact title first; if it exists, stop rather than duplicate.
5. **Verify read-only**: `search_files` the AI Context folder for exactly one correctly-named file, mimeType `application/vnd.google-apps.document` (a native Doc, not `text/markdown`). Flag any duplicate/leftover for human cleanup; never try to delete it.
...
When the master `client-index` Doc already exists (currently `1EVC4h1KAhr8EoAGDQxU8gFxCsnv9_n9TJ5uCWVc_KjA`) and you only need to add one new client's row, you CANNOT re-create the Doc (create-once). The index is a **native table in a native Google Doc** (per `../../shared/ai-context-file-contract.md` — edit cells via `editTableCell` / `findAndReplaceInDoc`, never `insertText`), so:
