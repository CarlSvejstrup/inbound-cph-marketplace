...
Editor imports. It reads two workbook dialects on one contract (it just recognizes both):
  - SETUP:     inb-ads-campaign-build assembler workbook — a full new campaign
             (Campaign settings, Ad groups, Keywords, Negatives, RSAs, Assets).
  - OPTIMIZE:  review_workbook — a subset (Negative keywords, promoted
...
builders run on either — one converter, no fork. It's the shared converter for BOTH the build path
(inb-ads-campaign-build) and the optimize path (inb-ads-optimization-loop) — all sibling skills in the same plugin,
so there is one export_csv.py, no per-plugin copy to drift.
...
# Hard Editor limits (RSA). Kept in sync with inb-ads-rsa-copy/sheet_layout.py FIELDS; the
...
def build_ads(rsa_rows):
    ...
    # The inb-ads-campaign-build assembler is all net-new and carries no #Original. The optimization
...
def main():
    ...
    ap.add_argument("--workbook", required=True,
                    help="the confirmed .xlsx (inb-ads-campaign-build assembler or inb-ads-optimization-loop review_workbook)")
    ...
    # recognizes both): (a) the inb-ads-campaign-build ASSEMBLER workbook (tabs "01 Campaign settings",
