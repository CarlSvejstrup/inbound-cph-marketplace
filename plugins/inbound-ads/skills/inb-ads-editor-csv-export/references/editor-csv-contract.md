...
Single source of truth for how `inb-ads-editor-csv-export` converts a **confirmed** Google Ads review
workbook into Google Ads Editor import CSVs. It reads **two dialects on one contract** (§Two
dialects): the `inb-ads-campaign-build` `assembler` workbook (full new campaign) and the
...
The CSV target schema below is **§5 of the assembler-contract** (`inb-ads-campaign-build/references/
...
`inb-ads-rsa-copy/sheet_layout.py`; cross-plugin import does not resolve in Cowork, so the
