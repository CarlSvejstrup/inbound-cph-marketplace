...
En **læse-skill**: henter per-asset data via Google Ads MCP (`run_custom_gaql`), bygger en farvekodet `.xlsx` fra `build-sheet.py` (openpyxl), og gemmer den til Drive (connector) eller lokalt. Samme xlsx-mekanik som `inb-ads-search-term-analyse` og `annoncetekster` — ingen `gws`, ingen Sheets-API, kører i Cowork OG lokalt.
...
For hver asset, udled `vinkel` (benefit / trust / urgency / CTA / feature / keyword-led / brand / location / garanti — samme taksonomi som `inb-ads-rsa-copy/references/headline-craft.md`). Brug asset-teksten.
...
> **Bemærk — loopet krydser flere skills:** `inb-ads-rsa-hygiene` og `inb-ads-rsa-copy` er nu søster-skills i samme plugin (`inbound-ads`). Fordi medium er manuel paste (ingen kode-/fil-kobling), virker loopet uændret — og fordi de bor i samme plugin, er der intet krav om at installere flere plugins for at lukke build→operate→iterate-loopet.
