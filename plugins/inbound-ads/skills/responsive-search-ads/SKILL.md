...
**Når `responsive-search-ads` kaldes som under-trin af en anden skill (`campaign-build`, `annonce-optimering`) er AI Context allerede i kontekst — kør kun dette opslag når RSA-skillen køres standalone på en navngiven klient.** Som under-trin: spring opslaget over og brug den AI Context der allerede er loadet.
...
> **Bemærk — loopet krydser to skills, ikke to plugins:** `responsive-search-ads` og `annonce-optimering` (producenten) er nu søster-skills i samme plugin (`inbound-ads`). Mediet er stadig manuel paste i chatten, så koblingen er uændret løs — men der er ikke længere noget krav om at installere flere plugins for at lukke loopet.
