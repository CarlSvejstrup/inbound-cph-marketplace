...
The technique: tokenise every search term into 1-, 2-, and 3-word phrases, then AGGREGATE the
performance metrics (cost, clicks, conversions) across EVERY term that contains each n-gram. This
surfaces SYSTEMIC patterns that a per-term scan misses — especially in the cheap long tail:

    "gratis" appears in 40 terms × ~5 kr each = 200 kr, 0 conversions  -> one theme, not 40 terms
    "pris"   appears in 60 terms, strong conversions                   -> a winning theme

It is the complement to the server-side cost filter: the filter makes the expensive HEAD light;
the n-gram view recovers the TAIL's signal by summing it. `digest.py` calls this and splits the
result into waste-theme vs winner-theme candidates for the conversation.

Pure functions, no I/O. The SCRIPT owns the aggregation; the AGENT (with the human) judges which
n-grams are systemic waste vs winners — never the script.

Cost is DKK here (slim already converted micros->DKK). A small generic stoplist keeps function
words (og/til/i/for) from dominating the 1-grams.
"""
...
# the table. Danish + a few English; kept local so ngram.py is self-contained.
STOPWORDS = {
