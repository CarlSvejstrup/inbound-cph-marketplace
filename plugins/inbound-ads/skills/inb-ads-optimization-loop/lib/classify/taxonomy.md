...
field-tested taxonomy in `plugins/inbound-ads/_archive/search-terms/SKILL.md`
...
- This mirrors `inb-ads-rsa-hygiene`'s hard-won lesson (per-asset CVR is confounded +
  sub-significance) and `inb-ads-search-term-analyse`' `WINNER_MIN_CONV` / low-confidence flag.
...
`inb-ads-rsa-copy/references/headline-craft.md`:
