...
**genbruge det shippede `responsive-search-ads`-skill** én gang per ad group. Output (`rsa-manifest.json`
...
vinkel-taksonomi, headline-craft) bor i `responsive-search-ads` (`fill-sheet.py` + `references/
headline-craft.md`), aldrig duplikeret her. Dette er den ENE reference der peger UD til et andet skill —
det er bevidst: `responsive-search-ads` er script-tungt og genbruges på tværs af plugins (optimization-
...
`responsive-search-ads` er allerede den verificerede RSA-copy-motor (live tegntælling, rød over-længde-CF,
...
## Trin 1 — Kald responsive-search-ads per ad group

For hver ad group i `structuring.json`: kør `responsive-search-ads`-skillet med ad group'ens URL, vinkler
...
- **Ingen copywriting-regler her.** De bor i `responsive-search-ads`. Ændrer Google grænserne eller
  ændrer vinkel-taksonomien sig: ret `responsive-search-ads`, ikke denne fil.
