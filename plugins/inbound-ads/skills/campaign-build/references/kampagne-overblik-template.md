...
2. **Account → Import → From file** og vælg CSV-filerne (Editor importerer CSV, ikke .xlsx — denne workbook er review-laget; Editor-CSV'erne genereres fra den godkendte workbook af `editor-csv-export`-skillen).
...
- The import steps describe the CSV path even though v1 of the assembler emits Excel-only:
  the CSVs come from the `editor-csv-export` skill off the confirmed workbook. State that
