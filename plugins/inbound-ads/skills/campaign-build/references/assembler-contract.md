...
Excel, by the `editor-csv-export` skill. The assembler's job is therefore to
...
review-only metadata. It is the rich superset. The `editor-csv-export` skill is where the
...
This is what the `editor-csv-export` skill produces from the confirmed Excel. Kept here so
