...
`editor-csv-export` skill. The workbook is therefore a lossless superset — every
...
# header row (the editor-csv-export skill reads headers from row 1) — we style cells only,
...
def write_overview(path, strategy, structuring, rsa_manifest, assets, date, adless, failures):
    ...
    lines += [
        ...
        "workbook af `editor-csv-export`-skillen.)",
...
def main():
    ...
    print(json.dumps({
        ...
        "note": "Excel-only. The workbook is the client-confirmation artifact; Editor CSVs are "
                "generated later from the confirmed Excel by the editor-csv-export "
