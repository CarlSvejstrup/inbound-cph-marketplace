...
description: Kør opstarts-analysen for en ny Google Ads-kunde - gå Inbounds Analysearbejdet-tjekliste (Search) igennem mod kontoen og aflever en .docx opstartsrapport i kundens Drive-mappe. Verificerer de 35 punkter READ-ONLY via Google Ads MCP (udvidelser, annoncetekster, tracking, kampagneindstillinger, struktur/navngivning, keywords/matchtyper/search terms/QS, budstrategier, audiences) og giver hvert punkt status (OK / kan forbedres / kritisk / mangler data) med tallet bag. Skriver ALDRIG til kontoen. Slår Drive-mappe + Ads-ID op i vault clients/*.md; findes mappen ikke, stopper den og spørger. Brug når brugeren siger "kør opstartsanalyse", "analyser den nye konto", "opstarts-tjekliste på [klient]", "gennemgå den nye kunde", "analysearbejdet for [klient]", "ny kunde-analyse", eller starter en ny Google Ads-kunde op. Svarer på dansk.
---
...
(det er `inb-ads-account-audit`) og ikke en optimering af en kørende konto
(det er en optimering, se skills som `inb-ads-optimization-loop`).
...
- (c3) **gå i dybden hvor det fortjener det** , for tunge moduler (især C annoncetekster, G keywords)
  ...
  (`inb-ads-optimization-loop` / `inb-ads-rsa-hygiene`) , MED forbehollet at de skills kræver at kontoen har
...
3. **Næste skridt:** de kritiske/forbedrings-punkter er råstoffet til den grundlæggende
   ...
   RSA-gennemgang: henvis til `inb-ads-optimization-loop` / `inb-ads-rsa-hygiene` (kræver kørselshistorik).
