...
Kør `../../shared/client-context-intake.md` som allerførste trin: identificér klienten, åbn
master-klientindekset i Drive, find klientens række (for delte mapper vælg det specifikke
marked/konto), og træk AI Context-filen ind som ground truth. For en ny kunde findes rækken/filen
ofte ikke endnu — det er forventet: flag hullet tydeligt ("[klient] har ingen AI Context-fil endnu
— forventet for en ny kunde; jeg kører på det jeg kan samle fra Drive-mappen + Ads MCP") og fortsæt.
Spring aldrig opslaget stiltiende over.
...
2. **Analysegrundlag** — default **Struktur-gennemgang** (ren opsætnings-/hygiejne-tjek, virker
   uden historik). Tilbyd også `Med 90-dages data hvor det findes`. Quality Score pulles efter den
   fælles kontrakt i `../../shared/quality-score-pull.md` (QS = keyword-grain, landingsside er et
   FLAG ikke en score, recommend-only): brug `get_quality_score_audit(date_range="LAST_90_DAYS")`
   — `LAST_90_DAYS` er kun gyldigt som tool-argument, IKKE som rå GAQL-`DURING`/`WHERE`-literal
   (der: `BETWEEN '<YYYY-MM-DD>' AND '<YYYY-MM-DD>'`).
...
(punkt 13-14), keyword-dækning (28), geo/sprog-korrekthed (21), brand-ekskludering (29). Kør derfor
den fælles Phase-0-grounding fra `../../shared/offering-brief.md` FØRST via én kort
offering-sub-agent: den bygger grundsandheden ud fra (1) landingsside-scrape af RSA `final_urls`
(Firecrawl) + (2) konto-signaler (kampagne-/ad-group-navne, eksisterende RSA-headlines), begge
krydstjekket, og returnerer en kompakt dansk profil (hvad de sælger, hvem, geografi/sprog,
brand-varianter, hvad der IKKE er en del af tilbuddet, `OFFERING_TOKENS`). Fejler scrape/for tyndt:
brug kun konto-signaler og sig det i `Kilde`-linjen — fabrikér aldrig et tilbud. Profilen er intern
kontekst, bygges frisk hver kørsel.
...
- (b) for hvert punkt afgøre `status` efter doms-reglen i reference-filen, kopiere `kind`
  (`lookup`/`judgment`) fra reference-filens kolonne (gæt ikke), og skrive en kort dansk `finding`
  med det faktiske tal/navn bag — aldrig en påstand uden data,
- (c) lokalisere konkrete fund i `evidence` (kampagne › ad group › asset › den nøjagtige streng),
  fylde `details` for tunge moduler (C annoncetekster, G keywords), og sætte `pointer` til det rette
  dybde-skill hvor en fuld gennemgang sprænger opstartsrapporten — men altid give top-N det værste
  først, `pointer` erstatter aldrig et reelt fund,
- (d) returnere præcis den form `build_docx.py` læser. **Feltbetydninger (`status`/`kind`/`finding`/
  `evidence`/`details`/`pointer`) + den fulde JSON-form: se `references/findings-schema.md`** — den
  kanoniske kontrakt, som `build_docx.py` også forbruger.
...
Saml de ni modul-objekter + offering-noter til ét findings-objekt efter **den kanoniske form i
`references/findings-schema.md`** (samme kontrakt `build_docx.py` forbruger). Tre ting styrer
kvaliteten af samlingen:

- `generated` sætter du (dagens dato — scriptet kalder aldrig en ur-funktion).
- `headline_findings` (3-5 vigtigste fund, prioritér `critical` > `warn`, skarpt og konkret) er
  den eneste redaktionelle vurdering du sender ind — overbliks-tallene beregner scriptet selv fra
  punkternes `status`, så båndet aldrig kan modsige rækkerne.
- `modules`: giv sub-agenternes `evidence`/`details`/`pointer` videre ORDRET — omskriv eller
  komprimér dem aldrig. Du må stramme `finding`-prosaen let, men `evidence` er adresser og skal stå
  præcist (se Troubleshooting for hvorfor).
...
- **`evidence` tabt ved hånd-samling (reel fejl-fælde, live-kørsel 1).** Søgeterm-agenten skrev
  "stavefejl … i annoncegruppen Aalborg", men ved manuel samling faldt "i annoncegruppen Aalborg"
  væk, så fundet ikke kunne handles på. Kopiér `evidence`-felterne 1:1 ind i det samlede objekt —
  det er adresser, ikke prosa.
- **Tomt GAQL-svar på en frisk konto** betyder oftest ingen historik, ikke "findes ikke aktivt" →
  `no_data`, aldrig et fabrikeret fund.
...
- Findings-kontrakten bor i `references/findings-schema.md` — én kilde, som både denne SKILL.md og
  `lib/build_docx.py` (docstringen øverst) peger på. Ændrer kontrakten sig, ret skema-filen og
  scriptets kode i takt.
- `.docx`-layoutet bor i `lib/build_docx.py` (self-bootstrapper python-docx).
