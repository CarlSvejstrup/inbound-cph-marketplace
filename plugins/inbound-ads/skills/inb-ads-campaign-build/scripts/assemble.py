...
inb-ads-rsa-copy/sheet_layout.py (that kept the assembler coupled to another skill for the
sake of three never-changing integers). The same three values are mirrored in
inb-ads-rsa-copy/sheet_layout.py FIELDS; if Google ever changes a cap, update both. Guard 2
...
# inb-ads-rsa-copy/sheet_layout.py, so it can no longer rely on that module's bootstrap to
...
# skill. Canonical mirror of the same three values: inb-ads-rsa-copy/sheet_layout.py FIELDS.
