...
self-contained — it does NOT import `inb-ads-rsa-copy/sheet_layout.py` (decoupled: a
...
- **No live-Cowork end-to-end run** + **`05-rsa-copy` intake-injection assumption** (the
  reference injects per-ad-group intake into the `inb-ads-rsa-copy` skill) — both remain
