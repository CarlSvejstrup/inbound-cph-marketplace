...
Last updated: 2026-05-27, after v0.7.0.
...
- `search-terms` — focused Google Ads search terms analysis for one client (v0.7.0). Read-only: pulls `search_term_view` via GAQL (cost-desc, ENABLED only, 90-day default) with the triggering keyword segment, pulls the full `keyword_view` map for placement detection, and scrapes the client's landing page via Firecrawl to ground relevance calls. Classifies every term into five buckets — RELEVANT, VINDER (converts, not yet exact -> promote), FORKERT_PLACERET (already a keyword in a different ad group -> negative here), IRRELEVANT (not in the client's offering -> negative), GRAENSE (manual). Synthesises an import-ready negative-keyword list. Output is an eight-tab colour-coded `.xlsx` (Oversigt, Alle search terms, the five per-bucket tabs, Anbefalede negatives) built by `build-sheet.py` (openpyxl), uploaded via the Drive connector or saved locally — same xlsx mechanic as rsa-copy, no `gws`/Sheets API, runs in Cowork. Taxonomy + layout + colours adopted from a field-tested user template (Dansk Studie Center), with VINDER + per-term aggregation + ENABLED-only filter + CPA handling added on top. GAQL fields (incl. `segments.keyword.info.*`, `keyword_view`, ctr-as-fraction) and the micros/grain handling verified against a live account; `build-sheet.py` smoke-tested across all eight tabs. Not yet run end-to-end against a full real account with the Firecrawl + LLM passes.
