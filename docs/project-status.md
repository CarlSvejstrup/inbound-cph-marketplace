...
Last updated: 2026-07-01, after the campaign-build restructure (3 shells removed → 12 skills) + direct-to-Ads delivery.

> **2026-07-01 — campaign-build restructure + direct-to-Ads delivery (`inbound-ads` v3.3.0, current roster).** Two changes landed. **(1) Three thin standalone shells were REMOVED** — `inb-ads-campaign-research`, `inb-ads-campaign-structure`, `inb-ads-campaign-assets`. Their logic already lived inside `inb-ads-campaign-build` as `references/` (the Phase 1→4 pipeline), and they were just solo-mode wrappers around those references, so they were deleted; the phases now live ONLY as references inside `inb-ads-campaign-build`. This drops the roster from 15 to **12 skills**. `inb-ads-campaign-build` and `inb-ads-rsa-copy` remain the BUILD group. **(2) `inb-ads-campaign-build`'s delivery model changed, reversing the 2026-06-03 "never pushes to the Google Ads API" decision.** New default: it **creates the campaign directly in the Google Ads account** via the `ads-writer` agent, HITL-gated per action, started **paused** (safe, recommended) or **active** per the user's choice. The 10-tab Excel review workbook is now **opt-in** — used only when the client must approve the setup first (then `inb-ads-editor-csv-export` makes the Editor CSVs, unchanged). Budget writes remain gated behind the write-guardrail hook + `INBOUND_ADS_BUDGET_GUARDRAIL`, requiring explicit per-action confirmation. Because it now writes to client accounts, changes to `inb-ads-campaign-build` require Ian's CODEOWNERS review. Current 12-skill roster: **Build** — `inb-ads-campaign-build`, `inb-ads-rsa-copy`; **Optimize** — `inb-ads-search-term-analyse`, `inb-ads-rsa-hygiene`, `inb-ads-optimization-loop`, `inb-ads-display-placement-audit`; **Standalone** — `inb-ads-account-audit`, `inb-ads-change-log`, `inb-ads-editor-csv-export`, `inb-ads-context-publish`, `inb-ads-context-update`, `inb-ads-onboarding-analysis`. The bundled agents (`ads-analyst`, `ads-writer`, `drive-knowledge`) and the write-guardrail hook are unchanged; `ads-writer` is the write path campaign-build now uses. Docs synced (README, repo-root CLAUDE.md, `plugin.json` description, this file, `session-handoff.md`). Dated notes below name the 3 now-removed shells and the old 15-skill count as they were at the time — kept verbatim as historical record; do not retro-edit them.
